...
- (optional) `NEXT_PUBLIC_CHATKIT_WORKFLOWS` — Comma-separated `Label:wf_xxx` pairs (for example `Support Bot:wf_123,Sales Coach:wf_456`) to populate the workflow picker in the UI. The first entry becomes the default workflow.
...
Set `NEXT_PUBLIC_CHATKIT_WORKFLOWS` to a comma-separated list of `Label:wf_xxx` pairs to populate the workflow picker shown above the chat experience. The first workflow in the list is selected by default, and switching the dropdown restarts ChatKit with the newly selected workflow without requiring a redeploy.
### 5. Deploy your app
