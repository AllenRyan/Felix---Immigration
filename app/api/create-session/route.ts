...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const knowledgeBaseId =
      parsedBody?.knowledgeBaseId && typeof parsedBody.knowledgeBaseId === "string"
        ? parsedBody.knowledgeBaseId.trim()
        : null;
    const { userId: baseUserId, sessionCookie: resolvedSessionCookie } =
      await resolveUserId(request);
    const scopedUserId = scopeUserId(baseUserId, knowledgeBaseId);
...
async function resolveUserId(request: Request): Promise<{
  ...
  if (error || !user) {
    // No authenticated user - this shouldn't happen due to middleware
    // but return error to force login
...
function scopeUserId(baseUserId: string, knowledgeBaseId: string | null): string {
  if (!knowledgeBaseId) {
    return baseUserId;
  }
  // Restrict to safe characters for upstream APIs and cookies
  const sanitizedKb =
    knowledgeBaseId.replace(/[^a-zA-Z0-9_-]/g, "").toLowerCase() || "defaultkb";
  return `${baseUserId}::kb_${sanitizedKb}`;
}
